...
4. Append a token usage block (e.g., `Claude: 12,345 tokens`, `Gemini: 2,000 tokens`, `Total tokens: 14,345`) to the session note; `npm run baw:workflow:sync` will ingest it automatically (use `npm run baw:token:auto -- --path ...` only when you need to re-run imports manually).
...
- `npm run baw:workflow:sync` now runs the importer automatically, scanning `ai-docs/capabilities/<capability>/sessions/` (and legacy `ai-docs/workflow/features`) for `Token usage:` blocks, appending totals to the ledger, and skipping files it has already ingested.
- If you need to target a specific directory manually, run:
