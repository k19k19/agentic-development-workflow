...
- Tool delegation strategy (Codex orchestrator delegating implementation to Claude MCP and research to Gemini MCP)
...
- `/quick [task]` – Direct implementation via Claude MCP (~5K tokens, <10 files) with Codex orchestrating
...
All implementation phases rely on Claude MCP (`mcp__claude__complete`) for code edits. Codex orchestrates the workflow, reviews Claude MCP output, and handles approvals/communication, while Gemini MCP supports large documentation or research pulls.
...
4. When a command enters an implementation phase, immediately delegate edits to Claude MCP and keep Codex focused on oversight, Gemini lookups, and reporting.
...
- Per-model breakdown (Claude/Gemini MCP usage vs Codex orchestrator time)
...
**Codex Orchestrator Handles:**
- Multi-agent coordination across scout/plan/build workflows
- Approval gates, reporting, and communication
- Routing work between Claude MCP (implementation) and Gemini MCP (research)

**Use Claude MCP for:**
- Small single-file implementations (`/quick`)
- CRUD boilerplate and UI component tweaks
- Syntax fixes and focused refactors that need direct code editing
- Deep analysis that benefits from Anthropic safety checks
...
- Delegate appropriately (Codex orchestrates, Claude MCP implements, Gemini MCP researches)
