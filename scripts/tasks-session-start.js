...
const CHEAP_MODELS = ['gemini-2.5-pro', 'gemini-2.5-flash', 'gemini-1.5-flash', 'gemini-1.5-pro', 'claude-sonnet-4-5', 'gpt-4o-mini'];
...
function printRecommendedTasks(recommended, remainingTokens) {
  if (!recommended.length) {
    ...
    console.log('Let Codex orchestrate: send research/doc reviews to Gemini MCP and implementation changes to Claude MCP to conserve tokens.');
    return;
  }
  ...
  console.log('Route simple doc summaries to Gemini MCP and UI/code tweaks to Claude MCP so Codex stays focused on complex coordination.');
}
...
(async () => {
  try {
    ...
    if (weeklyUsed > 0) {
      console.log(`Model mix (last 7 days): ${cheapShare.toFixed(1)}% from Gemini/Claude tiers.`);
      if (cheapShare < 60) {
        console.log('Tip: Shift simple discovery to Gemini MCP and boilerplate edits to Claude MCP so Codex stays within the $20 plan.');
      } else {
        console.log('Great balance! Continue leaning on Gemini/Claude for straightforward tasks to maximize Codex availability.');
      }
    } else {
      console.log('No token usage recorded yet. Default to Gemini MCP for document reads and Claude MCP for quick fixes while Codex coordinates.');
    }
    ...
    console.log('2. Default to Gemini MCP for doc summarization/research and Claude MCP for UI or syntax fixes while Codex orchestrates.');
    console.log('3. Keep Codex focused on architecture, multi-file reasoning, and verification while Claude MCP executes edits.');
