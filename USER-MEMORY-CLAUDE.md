...
- Delegate to specialized tools (Gemini MCP, Codex, Chrome DevTools)
...
| Boilerplate, configs | Codex | Quick syntax, no context needed |
| Fix syntax bugs | Codex | Immediate, no architectural context |
| UI/UX work | Codex | Specializes in component code |
...
Step 1: Identify scope
  - Single file bug? → Codex
  - Multi-file bug? → Codex orchestrator
  - Config/env issue? → Codex
...
3. Use Codex for boilerplate examples
...
- Production code (delegate to Codex via Codex)
...
**Not for:**
- Unit tests (delegate to Codex)
- Backend API tests (delegate to Codex)
...
**Not for:**
- Simple boilerplate (delegate to Codex)
...
3. Optimize:
   ...
   - Plan: Simplify scope
   - Build: Tighten implementation scope and lean on Gemini for research so Codex edits stay efficient
