...
| --- | --- | --- |
| **Intake** | `feature-manifest.json`, `intake/requirements.md` | `npm run scaffold:feature -- --title "Fleet Dispatch" --token 20000 --owner ops`
...
| **Build / Test** | `builds/BUILD-*.log`, `reports/REPORT-*.md` | `/build` or `npm run workflow:sync` (after each build)
...
3. Invoke `npm run workflow:sync` after every build/report pair so the dashboard reflects the new state.
...
- **Start of day:** run `npm run tasks:session-start` to view the backlog. Open the relevant feature directory and read `session-backlog.json`.
...
- **End of day:** update `session-backlog.json` with remaining tasks, run `npm run workflow:sync`, and attach the latest ledger decisions.
